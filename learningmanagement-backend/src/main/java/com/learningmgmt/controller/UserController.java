...
public class UserController {
	...
	public ResponseEntity<AuthenticationResponse> login(@RequestBody AuthenticationRequest authenticationRequest) {
		...
		UserDetails userdetail = authenticate(email,password);
		
		//GENERATE TOKEN ON THE BASIS OF USERDETAILS OBJECT
		String token = jwtToken.generateToken(userdetail);//New Era
	...
	private UserDetails authenticate(String email, String password) {
		UsernamePasswordAuthenticationToken emailpass = new UsernamePasswordAuthenticationToken(email, password);
		Authentication authenticate = authenticationManager.authenticate(emailpass);
